...
it('replaces variants and re-schedules when scheduledFor is set', async () => {
  ...
  expect(tx.post.update.mock.calls[0][0].data.status).toBe(PostStatus.SCHEDULED);
});
...
it('resets a FAILED post to DRAFT when edited with no schedule', async () => {
  ...
  expect(tx.post.update.mock.calls[0][0].data.status).toBe(PostStatus.DRAFT);
  expect(tx.post.update.mock.calls[0][0].data.scheduledFor).toBeNull();
});
